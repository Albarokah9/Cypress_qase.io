...
class LoginPage {
    ...
    assertInvalidCredentialsMessage() {
        cy.get(SELECTORS.alertMessage).should(
            'contain',
            'Invalid email address or password'
        );
        cy.screenshot('Login Failed with Invalid Credentials', {
            capture: 'fullPage',
        });
    }

    asserInvalidEmailFormatMessage() {
        cy.get(SELECTORS.invalidFeedback).should(
            'contain',
            'Please enter a valid email address'
        );
        cy.screenshot('Login Failed with Invalid Email Format', {
            capture: 'fullPage',
        });
    }
    ...
    assertRequiredEmailMessage() {
        ...
        cy.screenshot('Login Failed with Required Email', {
            capture: 'fullPage',
        });
    }
    ...
    assertRequiredPasswordMessage() {
        ...
        cy.screenshot('Login Failed with Required Password', {
            capture: 'fullPage',
        });
    }
    ...
    assertBothFieldsRequiredMessage() {
        ...
        cy.screenshot('Login Failed with Both Fields Required', {
            capture: 'fullPage',
        });
    }
